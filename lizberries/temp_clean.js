...
// Magnific Popup sarà inizializzato dopo il caricamento delle immagini
...
function loadImages() {
    ...
    if (images.length > 0) {
        images.forEach(image => {
            // Crea path per thumbnail (carousel) e full-size (popup)
            const thumbImage = image.replace('.webp', '_thumb.webp');
            const fullImage = image; // Full-size per il popup
            
            const imgElement = `
                <div class="item">
                    <a href="${fullImage}" class="fancylight popup-btn" data-fancybox-group="light"
                    aria-label="Visualizza l'immagine ${picCount} della galleria fotografica">
                        <img class="img-fluid max-height-155" src="${thumbImage}" alt="" loading="lazy">
                    </a>
                </div>
            `;
    ...
    // Inizializza Magnific Popup DOPO che le immagini sono state caricate
    setTimeout(function() {
...
function loadPressImages() {
    ...
    if (imgPress.length > 0) {
        imgPress.forEach((prsimage, index) => {
            ...
            // Crea path per thumbnail (preview) e full-size (popup)
            const thumbImage = prsimage.replace('.webp', '_thumb.webp');
            const fullImage = prsimage; // Full-size per il popup
            
            //console.log(`Image ${index + 1}: ${prsimage}, Rotation: ${rotation}, Alt Text: ${altText}`);

            const prsimgElement = `
                <div class="polaroid" style="--rotation: ${rotation}">
                    <a href="${fullImage}" class="image-link" aria-label="Visualizza l'immagine ${picPressCount} della sezione Press">
                        <img src="${thumbImage}" alt="${altText}" class="img-fluid">
                    </a>
        ...
        musicopolizForm.addEventListener('submit', async function(event) {
            ...
            try {
                ...
                const response = await fetch(MUSICOPOLIZ_WORKER_URL, {
                    ...
                    body: JSON.stringify({ 
                        password: password,
                        source: 'lizberries' // Identifica la sezione del sito
                    })
                });
